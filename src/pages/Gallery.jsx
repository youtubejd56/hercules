...
export default function Gallery() {
  ...
  return (
    <div className="p-8 md:p-12 animate-slide-up max-w-7xl mx-auto py-6 w-full min-h-[calc(100vh-80px)]">
      <header className="mb-10 text-center">
        <h2 className="md:text-5xl text-3xl font-extrabold mb-3 text-white">Gym Gallery</h2>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {images.map((img) => (
            <div key={img.id} className="group relative rounded-2xl overflow-hidden bg-card shadow-[0_20px_40px_rgba(0,0,0,0.4)] transition-all duration-500 hover:-translate-y-2 hover:shadow-[0_20px_40px_rgba(255,62,62,0.15)] border border-white/5">
              <img
                src={img.image?.startsWith('res.cloudinary.com') ? `https://${img.image}` : img.image}
                alt={img.title}
